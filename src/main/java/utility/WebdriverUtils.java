...
import exceptions.InvalidElementTypeException;
import exceptions.TooFewWindowsException;
import exceptions.TooManyWindowsException;
import exceptions.UnableToFindWindowException;
...
public class WebdriverUtils {
    ...
    public static WebDriver openBrowser() throws Exception {
        ...
        try {
            ...
            System.out.println("Browsers Get: " + Browsers.get());
            System.out.println("Env value: " + System.getenv("browser"));
            System.out.println("Properties File: " + Utils.getProperty("api.browser"));
            ...
            switch (currentBrowser) {
                ...
                case SAUCELABS:
                    String[] tags = {"Clinverse", "IE", "Windows 7"};
                    DesiredCapabilities caps = DesiredCapabilities.internetExplorer();
                    caps.setCapability("platform", "Windows 7");
                    caps.setCapability("version", "10.0");
                    caps.setCapability("name", getTestName());
                    caps.setCapability("tags", tags);
                    caps.setCapability("build", "2015.5x.W2");
                    // Create the connection to Sauce Labs to run the tests
                    driver = new RemoteWebDriver(
                            new URL("http://" + "moises_siles" + ":" + "ae252335-76d5-4b4b-9720-271f3927a9d5" + "@ondemand.saucelabs.com:80/wd/hub"),
                            caps);
    ...
    public static void goToURL(String url) throws Exception {
        try {
            ...
            driver.get(fullUrl);
            Log.info("Going to " + fullUrl, true);
        } catch (Exception ex) {
            captureScreenShot(driver, new Exception().getStackTrace()[0].getMethodName());
    ...
    public static int lookForRowIndex(int rowNumber, String gridName, Filter[] filters) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < filters.length; i++) {
                ...
                String dataValue = filters[i].dataFieldName != null ? ExcelUtils.getCellData(rowNumber, filters[i].dataFieldName) : filters[i].value;
            ...
            String rowID = (String) ((JavascriptExecutor) driver).executeScript(jsCode);
            jsCode = "return $('" + gridName + "').grid.getRowIndex('" + rowID + "');";
            rowIndex = ((Long) ((JavascriptExecutor) driver).executeScript(jsCode)).intValue();

            // Get the rowIndex

        } catch (Exception ex) {
            captureScreenShot(driver, new Exception().getStackTrace()[0].getMethodName());
            Log.error("Class WebdriverUtils | Method lookForRowIndex | Exception occurred while executing javascript. Code: "
                    + jsCode + "   Exception: " + ex.getMessage());
            throw ex;
        }
    ...
    public static Boolean selectGridRow(int rowNumber, String gridName, Filter[] filters) throws Exception {
        ...
        try {
            ...
            WebdriverUtils.waitForInvisibilityOfElementLocated(By.id(gridName + "Load"));

            rowIndex = WebdriverUtils.lookForRowIndex(rowNumber, gridName, filters);
            if (rowIndex >= 0) {
                WebdriverUtils.selectGridRow(gridName, rowIndex);
                founded = true;
            } else {
                ...
                for (int i = 0; i < filters.length; i++) {
                    ...
                    if (filters[i].moveTo == 0) {
                        if (filters[i].dataFieldName != null && !filters[i].dataFieldName.isEmpty()) {
                            ...
                            WebdriverUtils.sendKeys(rowNumber, WebdriverUtils.getTxtFilter(gridName, filters[i].columnHeader), filters[i].dataFieldName);
                        } else if (!filters[i].value.isEmpty()) {
                            WebdriverUtils.sendKeys(WebdriverUtils.getTxtFilter(gridName, filters[i].columnHeader), filters[i].columnHeader, filters[i].value);
                        }
                    }
                }
                Thread.sleep(1000);  // Wait for loading message to be showed
            ...
            captureScreenShot(driver, new Exception().getStackTrace()[0].getMethodName());
            Log.error("Class WebdriverUtils | Method selectGridRow(String gridName, Filter[] filters) | Exception: " + ex.getMessage());
            throw ex;
        }
    }
